...
// TD-11: единый источник правды для адресов наших контрактов — foundry broadcast run-latest.json
val deploymentConfigDir = layout.buildDirectory.dir("generated/source/deploymentConfig/java")

val generateDeploymentConfig by tasks.registering {
    val chainId = (project.findProperty("CHAIN_ID") as? String)?.toLongOrNull() ?: 97L
    val jsonFile = file("../contracts/broadcast/Deploy.s.sol/$chainId/run-latest.json")
    if (jsonFile.exists()) inputs.file(jsonFile) // JSON может отсутствовать (dev/staging) — тогда генерим 0x0
    outputs.dir(deploymentConfigDir)
    doLast {
        val addresses = mutableMapOf<String, String>()
        if (jsonFile.exists()) {
            val root = groovy.json.JsonSlurper().parse(jsonFile) as Map<*, *>
            for (tx in (root["transactions"] as? List<*>) ?: emptyList<Any?>()) {
                val m = tx as? Map<*, *> ?: continue
                if (m["transactionType"] == "CREATE") {
                    val name = m["contractName"] as? String ?: continue
                    val addr = m["contractAddress"] as? String
                    if (!addr.isNullOrBlank()) addresses[name] = addr
                }
            }
        } else {
            println("WARN: broadcast JSON not found: $jsonFile — generated addresses will be 0x0")
        }
        val ZERO = "0x0000000000000000000000000000000000000000"
        fun find(name: String): String = addresses[name]?.takeIf { it != "0x0" } ?: ZERO
        val socialRecoveryModule = find("SocialRecoveryModule")
        val smartAccountFactory = find("MDAOSmartAccountFactory")
        if (socialRecoveryModule == ZERO) println("WARN: SocialRecoveryModule not found in $jsonFile — using 0x0")
        if (smartAccountFactory == ZERO) println("WARN: MDAOSmartAccountFactory not found in $jsonFile — using 0x0")
        val outFile = deploymentConfigDir.get().asFile.resolve("com/mdaopay/app/core/blockchain/DeploymentConfig.kt")
        outFile.parentFile.mkdirs()
        outFile.writeText(
            """
            package com.mdaopay.app.core.blockchain

            // GENERATED by :app:generateDeploymentConfig from foundry broadcast (Deploy.s.sol run-latest.json) — DO NOT EDIT.
            object DeploymentConfig {
                const val SOCIAL_RECOVERY_MODULE = "$socialRecoveryModule"
                const val SMART_ACCOUNT_FACTORY = "$smartAccountFactory"
            }
            """.trimIndent() + "\n"
        )
    }
...
android {
    sourceSets {
        getByName("main") {
            java.srcDir(generateDeploymentConfig.map { it.outputs.files.singleFile })
        }
    }
...
// TD-11: DeploymentConfig.kt генерируется до любой компиляции
