 ...
 * - Дополнительная валидация и логгирование на бэкенде
 * - Защита от утечек через браузер клиента
 */
fun Route.moonPayProxy(moonpayApiKey: String, moonpaySecretKey: String, relayHmacSecret: String) {
    ...
    post("/moonpay-proxy/{path...}") {
        try {
            ...
            call.respond(HttpStatusCode(response.status.value), response.bodyAsText())
