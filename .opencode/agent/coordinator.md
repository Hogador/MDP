...
Время:           <минут>
Модели и токены по агентам:   (сводка за всю волну, один раз в конце)
                 coordinator:  <модель> — <N> токенов
                 implementer:  <модель> — <N> токенов
                 code-reviewer: <модель> — <N> токенов
                 verifier:     <модель> — <N> токенов
                 ...
                 Всего: <N> токенов
...
### Сводка моделей и токенов (один раз в конце волны)

После закрытия последней подзадачи волны составь **один сводный отчёт**
об использовании моделей. Не дублируй после каждого шага.

Формат:
```
Расход моделей за волну:
  coordinator:  <модель> — <N> токенов
  implementer:  <модель> — <N> токенов
  ...
  Всего: <N> токенов
...
Для каждого агента укажи модель (из swarm-конфига или task tool) и
токены (input + output). Если метрик нет — оцени по длине промта + ответа.
Явно напиши **Всего**.
